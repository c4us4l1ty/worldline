...
- [ ] **B-009 (P1) — SQL LWW lacks the in-memory tertiary tie-break.** `TableState::apply` arbitrates `(HLC, device, operation_id)` (PRD-DELTAS #65), but `apply_op_to_db` guards only `hlc_timestamp < op.hlc` (except `check_ins` which adds `id<`). Identical-HLC/different-op_id races (forked data dirs, same device) converge order-dependently in SQL. Mitigated today only by boot jitter (0–255 counter bump, `repo.rs`). Fix: extend SQL guards to `(hlc, operation_id)` ordering everywhere, backfill test with forked-DB fixture. (Largely closed commit `9a2eb99` via `record_heads` merge memory: `apply_op_to_db` now orders on `(hlc, device, operation_id)` against stored heads — `equal_hlc_ops_converge_regardless_of_delivery_order` green. OPEN RESIDUAL (new finding, 2026-09-18): exclusive composite-cursor pagination means an op whose HLC sits BELOW a replica's already-advanced cursor is permanently unreachable for that replica (test's zombie probe on device B proves it). One-device catch-up ordering is convergent; late-arriving below-cursor ops after a hard-stop are the gap. Fix requires arrival-sequence cursors or inclusive re-delivery — a SYNC-6-grade protocol change, deferred.)
